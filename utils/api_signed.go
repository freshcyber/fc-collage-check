...
// APIPhpSign api 签名规则 md5key为签名参数的key，signkey是salt
func APIPhpSign(mReq map[string]interface{}, signkey, md5key string) string {

	// fmt.Println("========STEP3, 在键值对的最后加上key=API_KEY========")
	//STEP1, 在键值对的最后加上key=API_KEY
	var _buffer bytes.Buffer //Buffer是一个实现了读写方法的可变大小的字节缓冲
	if signkey != "" {
		_buffer.WriteString(signkey)
	}
	...
	_md5Ctx.Write(_buf)
	_cipherStr := _md5Ctx.Sum(nil)
	upperSign := hex.EncodeToString(_cipherStr)
